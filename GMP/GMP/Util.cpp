...
std::set<int> Determine_2states(){
    ...
    //Print indices of the 2proc states used in the GL4 bound
   /* std::cout << "\n" << "2 procstates; ";
...
std::vector<bool> operator &&(std::vector<bool> State_nz, std::vector<bool> State_rowcol) {
    ...
    if (p == lengthState_row) {
        ...
        std::cerr << "Error in using && operator when determining New_state of NZ";
        std::vector<bool> Notassigned = { 0, 0, 0, 0 };
        return Notassigned;
    }
}
...
std::vector<bool> operator ||(std::vector<bool> vector1, std::vector<bool> vector2) {
    ...
    if (p == lengthState_row) {
        ...
        std::cerr << "Error in using || operator for vectors";
        std::vector<bool> Notassigned = { 0, 0, 0, 0 };
        return Notassigned;
    }
}
